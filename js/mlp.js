class MaterialListing {
  constructor() {
    this.materials = [
      ...
      "name=Pharisee & Tax Collector|desc=Parable of Jesus|author=RodTheNey|host=YouTube|id=Ufe6zouTq8g|type=Animation",
      "name=The Lost Son|desc=Parable of Jesus|author=RodTheNey|host=YouTube|id=mLI2zqMU6Ww|type=Animation",
      "name=Jesus Heals the Paralytic|desc=New Testament|author=RodTheNey|host=YouTube|id=0LqHYkGU4Zg|type=Animation",
      "name=Following Jesus|desc=New Testament|author=RodTheNey|host=YouTube|id=eKzGAlBvWP8|type=Animation",
      "name=The Good Samaritan|desc=Parable of Jesus|author=RodTheNey|host=YouTube|id=Dr0Vn5QBMtM|type=Animation",
      "name=Wise & Foolish Builders|desc=Parable of Jesus|author=RodTheNey|host=YouTube|id=CXWHLB1f6_U|type=Animation",
      "name=The Talents|desc=Parable of Jesus|author=RodTheNey|host=YouTube|id=bbPKhYBaWRg|type=Animation",
      "name=The Lost Sheep|desc=Parable of Jesus|author=RodTheNey|host=YouTube|id=tyWZeOlaRo4|type=Animation",
      "name=The Sower|desc=Parable of Jesus|author=RodTheNey|host=YouTube|id=V9IOhGPrRvY|type=Animation",
    ...
    this.expanded = () => this.materials.map(material => this.expand(material))

    this.youTube = id => `https://img.youtube.com/vi/${id}/0.jpg`
  ...
  expand(data) {
    var keyVals = data.split('|'), expanded = {}
    keyVals.map(keyVal => expanded[keyVal.split('=')[0]] = keyVal.split('=')[1])
    return expanded
  }

  build() {
    var html = ''
    this.expanded().map(mat => {
      html += `<a href="/pages/mdp.html" class="-mku"><div class="-inlineblock -vamiddle -img" style="background-image: url('${this.youTube(mat.id)}');"></div><div class="-details -inlineblock -vamiddle"><div class="-name">${mat.name}</div><div class="-desc">${mat.desc}</div><div class="-author">${mat.author}</div></div></a>`
    })
    this.mkus.innerHTML = html
  }
}
